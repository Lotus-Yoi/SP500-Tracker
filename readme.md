# SP500 Tracker

最初学习 Python 数据处理时，通过 pandas_exercises 练习基础操作。

但逐渐发现，比起单纯练习语法和 API，我更希望通过真实数据项目理解数据处理流程。

因此选择 S&P 500 作为第一个实践项目。

刚好最近在看 S&P 500，尽管大家都说定投标普这是一种傻瓜式投资，但我还是想自己看看怎么个事，所以就从这里开始。

## 目前实现

- 从 FRED 获取 S&P 500 数据
- 数据清洗与预处理
- 时间索引处理
- 数据去重与排序
- 计算每日收益率
- 自动检测并更新最新数据
- 网络请求失败自动重试（这里折腾了我半天 qwq）
- 使用 Matplotlib 画图
- GitHub Actions 自动提交更新后的`SP500.png`。

## 接下来

- 接入新闻与大模型API，生成每日市场分析报告

- 增加消息通知功能，将分析结果推送到个人设备

## 运行结果

![S&P 500 Chart](SP500.png)

## 更新日志

### V1.2
尝试让项目进行自动化运行，选择了GitHub Actions作为自动化运行环境。

- 在网络请求连续失败时加入进程退出码，使 GitHub Actions 能正确识别程序运行失败。

- 添加了`requirements.txt`，记录项目所需要的python第三方依赖。

- 添加了`.github/workflows/update.yml`，用于配置 GitHub Actions 自动运行流程。

- 为了适配 Ubuntu 运行环境，暂时移除绘图中的中文字体设置，并将图表文字改为英文。

- 实际运行发现首次运行时，由于不存在 `sp500.csv`，程序跳过了 `merge_data()`，导致不同运行路径产生的数据结构不一致。将 `daily_return` 计算移动到最终数据确定后执行，保证首次运行和更新运行逻辑一致。

- 重构`merge_data()`，将旧数据作为可选参数，使首次运行与后续更新均经过统一的去重、排序流程。

- 添加了按照时间表触发的自动运行任务--每日 UTC 1：30（北京时间9：30）执行。

- 实现了 GitHub Actions 自动提交更新后的`SP500.png`。

### V1.1 
`daily_return`这一项的第一行因为没有前一天的数据，本来就是`NaN`。
但是旧代码直接使用了`dropna()`导致了程序每运行一次都会删掉一行历史数据...

也就是说程序看起来是在正常运行，但数据其实在偷偷变少(悲)。

- 在gpt老师的帮助下，学会了一套数据处理逻辑————先确定可信的基础数据，然后进行合并、去重、排序，最后再进行计算。
- 同时完成了函数拆分，将程序拆分为 `fetch_data()`、`clean_data()`、`merge_data()`、`paint()` 和 `main()`

### V1.0
完成第一个可运行版本：
- 从 FRED 获取 S&P 500 数据
- 保存本地 CSV
- 计算每日收益率
- 使用 Matplotlib 绘制价格与收益率图